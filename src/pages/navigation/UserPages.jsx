...
const UserPages = () => {
  ...
  const [page, setPage] = useState(() => {
    const userPageNum = sessionStorage.getItem("userPageNum");
    return userPageNum ? parseInt(userPageNum) : 1;
  });
  const [totalPages, setTotalpages] = useState();
  const filterPost = pages.filter(
    (post) =>
      post.post !== null && (post.church !== null || post.branch !== null),
  );
  ...
  const fetchMorePosts = async () => {
    const res = await api.get(`/user-pages/${id}?page=${page}&limit=50`);
  ...
  useEffect(() => {
    fetchMorePosts();
    sessionStorage.setItem("userPageNum", page);
  }, [id, toggle, page]);
  return (
    <div>
      <button
      ...
      {filterPost.length > 0 &&
        filterPost.map((page) => (
          <div
            key={page._id}
            className="flex gap-2.5 px-4 py-3 border-t border-r"
          >
            <Link
              to={
                page.branch?._id
              ...
              {page.post !== null && (
                <div className="grid grid-cols-4 pt-3 gap-x-5 min-[800px]:gap-x-20">
                  <div className="flex gap-1">
                    ...
                    />
                    {page.post.comments.length > 0 && page.post.comments.length}
                  </div>
                  <div className="flex gap-1">
                    <LikePost
                      postID={`${page.post._id}`}
                  ...
                  <div className="flex gap-1">
                    <SavePost
                      ...
                      onToggle={Toggle}
                      booked={{
                        booked: page.post?.bookmark.includes(user)
