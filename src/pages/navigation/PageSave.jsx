...
const PageSave = () => {
  ...
  const [page, setPage] = useState(() => {
    const groupNum = sessionStorage.getItem("groupNum");
    return groupNum ? parseInt(groupNum) : 1;
  });
  ...
  useEffect(() => {
    fetchMorePosts();
    sessionStorage.setItem("groupNum", page);
  }, [toggle, page]);
  return (
    <div>
      <button
      ...
      {pages.map((page) => (
        <div
          key={`${page._id}`}
          className="px-4 py-3 border-t border-r flex gap-2.5"
        >
          <Link
            to={
              page.branch?._id
            ...
            {page.post !== null && (
              <div className="grid grid-cols-4 pt-3 gap-x-5 min-[800px]:gap-x-20">
                ...
                <div className="flex gap-1">
                  ...
                  />
                  {page.post?.bookmark.length > 0 && page.post?.bookmark.length}
                </div>
              </div>
            )}
          </div>
