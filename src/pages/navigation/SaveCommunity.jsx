...
const SaveCommunity = () => {
  ...
  const fetchPosts = async ({ pageParam }) => {
    return await api.get(`/community-save?page=${pageParam}&limit=4`);
  };
  const {
    data,
    status,
    ...
    queryKey: ["fellowposts", "savecommunity"],
    queryFn: fetchPosts,
    initialPageParam: 1,
      ...
      {data?.pages.map((group, i) => (
        <React.Fragment key={i}>
          {group.data.posts?.map((post, i) => (
            <div key={i} className="px-4 py-3 flex gap-2.5 border-b border-r">
