...
const EventOption = ({ eventId, url, event, urlDel, onToggle }) => {
  return (
    <Popover>
      <PopoverTrigger>
        <div>
      ...
      <PopoverContent className={"w-fit gap-y-3 flex flex-col p-2"}>
        <DelEvent eventId={eventId} url={urlDel} onToggle={onToggle} />
        <EditEvent eventId={eventId} event={event} url={url} />
      </PopoverContent>
    </Popover>
  );
};
...
const EditEvent = ({ eventId, url, event }) => {
  const eventData = {
